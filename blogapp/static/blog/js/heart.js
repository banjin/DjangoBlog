(function ($) {
		    ...
		    Garden.randomrgba = function (min, max, a) {
		    	//return Garden.rgba(Math.round(Garden.random(250, 255)), Math.round(Garden.random(200, 210)), Math.round(Garden.random(200, 210)), 0.05);//绱壊
		    	return Garden.rgba(Math.round(Garden.random(180, 250)), Math.round(Garden.random(80, 255)), Math.round(Garden.random(50, 180)), 0.15);//姗欒壊,绮夌孩,绱壊
		    };
			...
			var flag = 0;
			var time, temp = 800;
			var radian = Math.PI;//寮у害
			var radianDecrement = Math.PI / 370 * 2;;//寮у害澧為噺
			var heartX, heartY;
			var LX = 700, LY = 370;
			...
			function drawHeart(){
				...
				if(flag < 31){
					...
					else if(!(radian<=Math.PI+205*radianDecrement&&radian>=Math.PI+180*radianDecrement))//鍙槸璁╁彉寰楀ソ鐪�
						garden.createRandomBloom(heartX, heartY, 12);
				}
			}

			function getX(t) {//鐢卞姬搴﹀緱鍒� X 鍧愭爣
				return 305+r*(16*Math.pow(Math.sin(t),3));
			}
			 
			function getY(t) {//鐢卞姬搴﹀緱鍒� Y 鍧愭爣
